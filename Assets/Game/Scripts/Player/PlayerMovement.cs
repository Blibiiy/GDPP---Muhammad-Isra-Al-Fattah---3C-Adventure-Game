...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Move(Vector2 axisDirection)
    {
        ...
        bool isPlayerGroundMove = _playerStance == PlayerStance.Stand || _playerStance == PlayerStance.Crouch;
        bool isPlayerClimbing = _playerStance == PlayerStance.Climb;
        bool isPlayerGliding = _playerStance == PlayerStance.Glide;
        if (isPlayerGroundMove && !_isPunching)
        {
            ...
            switch (_cameraManager.CameraState)
            {
                case CameraState.ThirdPerson:
                    if (axisDirection.magnitude >= 0.1)
                    {
                        ...
                        _rigidbody.AddForce(movementDirection.normalized * Time.deltaTime * _speed, ForceMode.Acceleration);
                    }
                    break;
